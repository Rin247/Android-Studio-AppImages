...
# filename for the given version by scraping the stable releases page.
tarball=$(curl --fail -s -A "${curl_ua}" "https://developer.android.com/studio" \
    | grep -oE "ide-zips/${app_version}/android-studio-[a-zA-Z0-9_-]+-linux\\.tar\\.gz" \
    | head -n 1 \
    | sed -E "s#^ide-zips/${app_version}/##")
if [ -z "${tarball}" ]; then
    echo "error: could not resolve Android Studio tarball filename for version ${app_version} on https://developer.android.com/studio" >&2
    exit 1
fi
...
podman exec "${container_name}" bash -lc "export ARCH=x86_64; ${work_dir}/scripts/build.sh ${app_version} ${tarball}"
